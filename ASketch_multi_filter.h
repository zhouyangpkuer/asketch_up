#ifndef _ASKETCH_MF_H
#define _ASKETCH_MF_H
...
public:
	ASketch_mf(double memory, int _d, int _filter_size)
	{
		...
		memset(cur_pos, 0, sizeof(bucket_num));
	...
	{	
		int bucket_chose = key % bucket_num;

		const __m128i item = _mm_set1_epi32((int)key);

		__m128i *keys_p = (__m128i *)(items + (bucket_chose << 4));
		...
		if(matched != 0)
		{
			...
			int matched_index = _tzcnt_u32((uint)matched) + (bucket_chose << 4);
			new_count[matched_index] ++;
			return;
		}

		if(cur_pos[bucket_chose] != 16)
		{
			int temp_index = cur_pos[bucket_chose] + (bucket_chose << 4);
			items[temp_index] = key;
			new_count[temp_index] = 1;
			old_count[temp_index] = 0;
			cur_pos[bucket_chose] ++;
			return;
		}
		...
		min_index = 0;
		min_value = (1 << 30);
		for(int i = (bucket_chose << 4); i < (bucket_chose << 4) + 16; i++)
		{
